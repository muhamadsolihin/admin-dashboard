...
const onSubmit = () => {
  ...
  loading.value = true;
  http
    .put(`/vulture/v1/oklahoma/${route.params.uuid}`, formData)
    .then((response) => {
      const res = response.data;
      if (res.status) {
        showSuccessNotif("Karyawan berhasil diubah!");
...
onMounted(async () => {
  ...
  http
    .get(`/vulture/v1/hawaii/${route.params.uuid}`)
    .then((result) => {
      ...
      if (res.status) {
        ...
        typeUser.value = res.data.user_type;
        address.value = res.data.address;
        village.value = res.data.village_name;
        selectedOutlet.value = res.data.outlet_id.toString();
        selectedVillage.value = [res.data.village_id, res.data.village_name];
      }
    })
    .finally(() => {
...
function getEmployees() { throw new Error("Function not implemented."); }
