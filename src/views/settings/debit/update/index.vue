...
import DebitModule from "@/store/modules/DebitModule";
...
const DebitState = getModule(DebitModule);
...
const onSubmit = () => {
  loading.value = true;
  const debit = DebitState.updateDebit({
    uuid: uuid.value,
    name: bankName.value,
    ...
    .then((res) => {
      ...
      if (response.status) {
        ElNotification({
          ...
          message: "Debit berhasil diperbarui!",
        });
        router.push(`/setting/debit/list`);
      } else {
        ElNotification({
          title: "Error",
          type: "error",
...
onMounted(() => {
  DebitState.getDetailDebit({
    uuid: route.params.uuid,
    outletId: route.query.oid,
  }).then(() => {
    const detailDebit = DebitState.getDebit;
    uuid.value = detailDebit.uuid;
    bankName.value = detailDebit.name as any;
    numberCard.value = detailDebit.number as any;
    selectedOutlet.value = detailDebit.outlet_id?.toString() as string;
  });
