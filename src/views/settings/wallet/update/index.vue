...
const onSubmit = () => {
  loading.value = true;
  const debit = WalletState.updateWallet({
    uuid: uuid.value,
    name: bankName.value,
    ...
    .then((res) => {
      ...
      if (response.status) {
        ElNotification({
          ...
          message: "E-Wallet berhasil diperbarui!",
        });
...
onMounted(async () => {
  ...
  OutletState.getOutletsAPI({
    ...
    const detaiilWallet = WalletState.getterDetail;
    uuid.value = detaiilWallet.uuid;
    bankName.value = detaiilWallet.name as any;
    numberCard.value = detaiilWallet.number as any;
    selectedOutlet.value = detaiilWallet.outlet_id?.toString() as string;
  });
});
</script>
